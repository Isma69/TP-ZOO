...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Increment numberOfAnimal for the selected fence
    $selectedFence = $employee->findFenceById($fences_id);
    if ($selectedFence) {
        $selectedFence->incrementNumberOfAnimal();
        $employee->updateFence($selectedFence);
    }
                    ...
                    <div class="mb-1">
                        ...
                        <select id="fences_id" name="fences_id" class="form-select">
                            <?php foreach ($fences as $fence): ?>
                                <option value="<?php echo $fence->getId(); ?>"><?php echo $fence->getFenceType(); ?> <?php echo $fence->getNumberOfAnimal(); ?>/<?php echo $fence->getMaxCapacity(); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
